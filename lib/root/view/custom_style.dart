...
class CustomStyle {
  ...
  static Widget getChartFilterData({
    ...
    required Map<String, ThresholdData> thresholds,
  }) {
    int style = e['style'] ?? -1;
    int length = e['length'] ?? 1;
    String value = e['value'] ?? '';
    ...
    int status = e['status'] ?? 0;
    String id = e['id'] != null ? e['id'].toString() : '-1';
    double majorH = double.parse(e['MajorHI'] ?? -1.0);
    double minorH = double.parse(e['MinorHI'] ?? -1.0);
    double majorL = double.parse(e['MajorLO'] ?? -1.0);
    double minorL = double.parse(e['MinorLO'] ?? -1.0);
